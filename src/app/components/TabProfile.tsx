...
import comDefault from "../../assets/com-def.png"

export default function TabProfile({ profile, community }: { profile: { socialLinks: PlatformData[] }, community: { owned_communities: CommunitySummary[], member_communities: CommunitySummary[] } }) {
  const [tabType, setTabType] = useState<'social-link' | 'community'>('social-link')

  return (
    <>
      <div className="!bg-post-default flex items-center justify-center w-auto rounded-lg my-6">
        <div className="bg-post-default !p-0 h-auto flex items-center rounded-lg shrink-0">
          <button className={`space-x-2.5 h-8.5 rounded-lg px-4 cursor-pointer ${tabType === 'social-link' ? 'bg-post-active text-post-active-text' : 'bg-post-default text-type-text'}`}
          onClick={() => setTabType("social-link")}>
            <span className="text-sm font-medium">
              My Social Links
            </span>
          </button>

          <button className={`space-x-2.5 h-8.5 rounded-lg px-4 cursor-pointer ${tabType === 'community' ? 'text-gem-badge bg-question-active' : 'bg-post-default text-type-text'}`}
          onClick={() => setTabType("community")}>
            <span className="text-sm font-medium">
              Communities
            </span>
          </button>
      ...
      {tabType === 'social-link' && profile?.socialLinks?.length > 0 && (
        <div className="space-y-4 w-full">
          {profile?.socialLinks?.map((platform: PlatformData, index: number) => (
            <Link key={index} className="rounded-md border border-menu-btn bg-white h-16.5 p-2 flex items-center gap-1 w-full" href={platform?.userName || '#'} target="_blank" rel="noopener noreferrer">
              <div className="w-12 flex items-center justify-center gap-2">
                ...
                {platform.platform === "X (Twitter)" && twitterIcon}
                {platform.platform === "VK.com" && vkIcon}
                {platform.platform === "Pinterest" && pinterestIcon}
                {platform.platform === "Vimeo" && vimeoIcon}
                {platform.platform === "Discord" && discordIcon}
                {platform.platform === "Facebook" && facebookIcon}
                {platform.platform === "We Chat" && wechatIcon}
                {platform.platform === "Rakuten Viber" && viberIcon}
                {platform.platform === "WhatsApp" && whatsappIcon}
                {platform.platform === "Snapchat" && snapchatIcon}
                {platform.platform === "Notion" && notionIcon}
                {platform.platform === "Behance" && behanceIcon}
                {platform.platform === "Dribble" && dribbbleIcon}
                {platform.platform === "Flickr" && flickrIcon}
                {platform.platform === "LinkedIn" && linkedinIcon}
                {platform.platform === "Twitch" && twitchIcon}
                {platform.platform === "Reddit" && redditIcon}
                {platform.platform === "Slack" && slackIcon}
                {platform.platform === "Apple Music" && appleMusicIcon}
                {platform.platform === "Youtube Music" && youtubeMusicIcon}
                {platform.platform === "Sound Cloud" && soundcloudIcon}
                {platform.platform === "Shazam" && shazamIcon}
                {platform.platform === "Spotify" && spotifyIcon}
                {platform.platform === "PayPal" && paypalIcon}
                {platform.platform === "Stripe" && stripeIcon}
                {platform.platform === "Telegram" && telegramIcon}
                {platform.platform === "Github" && githubIcon}
                {platform.platform === "Gitlab" && gitlabIcon}
                {platform.platform === "Figma" && figmaIcon}
                {platform.platform === "Website" && websiteIcon}
              </div>
              <span className="text-sm font-medium text-pf-name">
                {platform?.platform}
              </span>
            </Link>
          ))}
      ...
      {tabType === 'community' && (
        <div className="space-y-4 w-full">
          {community?.owned_communities?.length > 0 && community?.owned_communities?.map((community: CommunitySummary, index: number) => (
            <Link key={index} className="rounded-md border border-menu-btn bg-white h-16.5 p-2 flex items-center gap-1 w-full" href={`https://www.pophop.chat/${community?.url_name}`} target="_blank" rel="noopener noreferrer">
              {community?.avatar ? (
                <Image src={community?.avatar} alt={`${community?.name} Avatar`} priority width={40} height={40} className="rounded-full object-cover" />
              ) : (
                <Image src={comDefault} alt="Community default" className="w-10 h-10 rounded-full" />
              )}
          ...
          {community?.member_communities?.length > 0 && community?.member_communities?.map((community: CommunitySummary, index: number) => (
            <Link key={index} className="rounded-md border border-menu-btn bg-white h-16.5 p-2 flex items-center gap-1 w-full" href={`https://www.pophop.chat/${community?.url_name}`} target="_blank" rel="noopener noreferrer">
              {community?.avatar ? (
                <Image src={community?.avatar} alt={`${community?.name} Avatar`} priority width={40} height={40} className="rounded-full object-cover" />
              ) : (
...
const vkIcon = (
  <svg fill="#000000" viewBox="-2.5 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg" className="w-10 h-10 shrink-0">
    <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
    <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
    <g id="SVGRepo_iconCarrier">
      <title>vk</title>
      ...
      <g id="7935ec95c421cee6d86eb22ecd12951c">
        <path
          ...
          d="M220.646,338.475C207.223,408.825,190.842,476.269,142.3,511.5
            ...
            C265.434-6.539,95.253,81.775,116.175,211.161c5.09,31.626,37.765,41.22,13.062,84.884
            c-57.001-12.65-74.005-57.6-71.822-117.533c3.53-98.108,88.141-166.787,173.024-176.293
      ...
      >
        ...
        >
          <g id="icons" transform="translate(56.000000, 160.000000)">
            <path
              d="M23.99,7403.93262 C23.9,7405.8454 22.541,7408.46538 19.911,7411.79058 C17.192,7415.26353 14.891,7417 13.009,7417 C11.843,7417 10.857,7415.94019 10.051,7413.82155 L8.437,7407.99457 C7.839,7405.87495 7.197,7404.81514 6.51,7404.81514 C6.36,7404.81514 5.837,7405.12441 4.941,7405.74296 L4,7404.55117 C4.986,7403.69722 5.957,7402.84326 6.913,7401.99029 C8.228,7400.87138 9.214,7400.28337 9.871,7400.22427 C11.425,7400.07751 12.381,7401.12156 12.74,7403.35839 C13.128,7405.77251 13.397,7407.2726 13.547,7407.8616 C13.995,7409.86303 14.488,7410.86374 15.026,7410.86374 C15.444,7410.86374 16.072,7410.21662 16.908,7408.92141 C17.745,7407.6262 18.193,7406.64026 18.253,7405.96359 C18.372,7404.84469 17.924,7404.28622 16.908,7404.28622 C16.43,7404.28622 15.937,7404.38865 15.429,7404.59451 C16.415,7401.44561 18.298,7399.91499 21.077,7400.00364 C23.138,7400.06274 24.11,7401.37174 23.99,7403.93262 Z"
              id="vimeo-[#144]"
            ></path>
          </g>
        </g>
...
const dribbbleIcon = (
  ...
  >
    ...
    <g id="SVGRepo_iconCarrier">
      ...
      <path
        className="cls-1"
        d="M8.12,2.24A32.47,32.47,0,0,1,12.8,9.13a33.23,33.23,0,0,1,1.58,3.56,36.16,36.16,0,0,1,2.15,8.79"
      ></path>
      <path
        className="cls-1"
        d="M19.67,4.82A18.16,18.16,0,0,1,12.8,9.13,28,28,0,0,1,1.55,11"
      ></path>
      <path
        className="cls-1"
        d="M22.42,13.21a16.66,16.66,0,0,0-8-.52A13,13,0,0,0,5.09,19.9"
      ></path>
    </g>
  </svg>
)
...
const soundcloudIcon = (
  <svg viewBox="0 0 192 192" xmlns="http://www.w3.org/2000/svg" fill="#000000" className="w-10 h-10 shrink-0">
    <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
    <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
    <g id="SVGRepo_iconCarrier">
      <path
        ...
        strokeLinejoin="round"
        strokeWidth="12"
        d="M14 95v28m24-41v48m24-57v58"
      />
      <path
        fill="#000000"
        d="M86 131h-6a6 6 0 0 0 6 6v-6Zm61.964-37.303-5.995-.257a6.001 6.001 0 0 0 9.01 5.444l-3.015-5.187ZM86 60.5l-3.542-4.843A6 6 0 0 0 80 60.5h6Zm23 64.5H86v12h23v-12Zm49 0h-49v12h49v-12Zm14-14c0 7.732-6.268 14-14 14v12c14.359 0 26-11.641 26-26h-12Zm-14-14c7.732 0 14 6.268 14 14h12c0-14.36-11.641-26-26-26v12Zm-7.021 1.884A13.91 13.91 0 0 1 158 97V85c-4.745 0-9.21 1.277-13.051 3.51l6.03 10.374ZM142 92c0 .483-.01.963-.031 1.44l11.989.513c.028-.648.042-1.299.042-1.953h-12Zm-33-33c18.225 0 33 14.775 33 33h12c0-24.853-20.147-45-45-45v12Zm-19.458 6.344A32.825 32.825 0 0 1 109 59V47c-9.917 0-19.1 3.215-26.542 8.658l7.084 9.686ZM80 60.5V131h12V60.5H80Z"
      />
    </g>
  </svg>
)
    ...
    <g id="SVGRepo_iconCarrier">
      ...
      >
        ...
        >
          <g id="icons" transform="translate(56.000000, 160.000000)">
            <path
              d="M99.915,7327.865 C96.692,7325.951 91.375,7325.775 88.297,7326.709 
                C87.803,7326.858 87.281,7326.58 87.131,7326.085 
                C86.981,7325.591 87.26,7325.069 87.754,7324.919 
                C91.287,7323.846 97.159,7324.053 100.87,7326.256 
                C101.314,7326.52 101.46,7327.094 101.196,7327.538 
                C100.934,7327.982 100.358,7328.129 99.915,7327.865 
                L99.915,7327.865 Z M99.81,7330.7 
                C99.584,7331.067 99.104,7331.182 98.737,7330.957 
                C96.05,7329.305 91.952,7328.827 88.773,7329.792 
                C88.36,7329.916 87.925,7329.684 87.8,7329.272 
                C87.676,7328.86 87.908,7328.425 88.32,7328.3 
                C91.951,7327.198 96.466,7327.732 99.553,7329.629 
                C99.92,7329.854 100.035,7330.334 99.81,7330.7 
                L99.81,7330.7 Z M98.586,7333.423 
                C98.406,7333.717 98.023,7333.81 97.729,7333.63 
                C95.381,7332.195 92.425,7331.871 88.944,7332.666 
                C88.609,7332.743 88.274,7332.533 88.198,7332.197 
                C88.121,7331.862 88.33,7331.528 88.667,7331.451 
                C92.476,7330.58 95.743,7330.955 98.379,7332.566 
                C98.673,7332.746 98.766,7333.129 98.586,7333.423 
                L98.586,7333.423 Z M94,7319 
                C88.477,7319 84,7323.477 84,7329 
                C84,7334.523 88.477,7339 94,7339 
                C99.523,7339 104,7334.523 104,7329 
                C104,7323.478 99.523,7319.001 94,7319.001 
                L94,7319 Z"
              id="spotify-[#162]"
            ></path>
          </g>
...
const stripeIcon = (
  <svg
    ...
    className="w-10 h-10 shrink-0"
  >
    <path d="M152.06738,152c0-6.69873-11.51953-10.16186-27.793-14.32324-8.93164-2.28394-18.16748-4.64575-25.63427-8.58838C84.834,121.79836,81.936,111.42141,81.936,104c0-20.52344,19.80322-36,46.064-36,18.60107,0,34.68164,7.93726,41.9668,20.71436a11.99982,11.99982,0,1,1-20.84864,11.88769C146.67676,96.32033,139.19971,92,128,92c-10.65527,0-22.064,4.822-22.064,12,0,4.21509,15.20166,8.1023,24.28417,10.42481,9.707,2.48217,19.74463,5.04907,28.03418,9.63623,14.72266,8.14746,17.813,19.69043,17.813,27.939,0,20.187-21.11377,36-48.06738,36s-48.06738-15.813-48.06738-36a12,12,0,0,1,24,0c0,2.19922,1.94824,4.8086,5.21191,6.98023C113.93848,162.17043,120.811,164,128,164s14.06152-1.82959,18.85547-5.01977C150.11914,156.80862,152.06738,154.19924,152.06738,152ZM228,48V208a20.02292,20.02292,0,0,1-20,20H48a20.02292,20.02292,0,0,1-20-20V48A20.02276,20.02276,0,0,1,48,28H208A20.02276,20.02276,0,0,1,228,48Zm-24,4H52V204H204Z"></path>
  </svg>
)
